...
# from app import app
#
# from models.medical import controller_info_by_student_id, consumption_by_student_id, get_student_info_by_student_id, \
#     get_grad_student_info_by_student_id, get_teachers_by_class_id, grade_query_res
# from models.medical import total_query_res
...
#                          children=dash_DropDown('controller-aspect-selector', '考勤情况与统计:', ['学生考勤情况', '学生考勤统计'],
#                                                 ['controller', 'controller-st'], 'controller'),
#                          style={'width': '40%', 'display': 'inline-block'}),
#
#                 html.Div(id='controller-select-chart',
#                          children=dash_DropDown('controller-graph-table-selector', '图表切换:', ['统计图', '统计表'],
#                                                 ['graph', 'table'], 'graph'),
#                          style={'width': '50%', 'display': 'inline-block', 'margin-left': '10px'}),
#                 html.Div(id='controller-select-term',
...
#                          children=dash_DropDown('consumption-graph-table-selector', '图表切换:', ['统计图', '统计表'],
#                                                 ['graph', 'table'], 'graph'),
#                          style={'width': '40%', 'display': 'inline-block', 'margin-left': '10px'}),
#                 html.Div(id='stua-select-interval',
#                          children=dash_DropDown('interval-selector', '年月日切换:', ['年数据', '月数据', '日数据', '总数据'],
#                                                 ['Year', 'Month', 'Day', 'Total'], 'Day'),
#                          style={'width': '40%', 'display': 'inline-block', 'margin-left': '10px', }, ),
#             ], style={'display': 'inline-block', 'vertical-align': 'middle', 'width': '90%'}),
#         ], className='one-row-con'),
...
#     grade = grade[
#         ['exam_name', 'subject', 'score', 'z_score', 't_score', 'r_score', 'div', 'Grank', 'Crank', 'Comment']]
#     return dash_table(header, grade.T, 'student-grade-table', '学生{0}历次考试成绩统计表'.format(stu_id),
#                       columnwidth_=[3, 2, 1, 1, 1, 1, 1, 1, 1, 1])
#
#
# @app.callback(
...
#     [Input('grade-subject-selector', 'value'), Input('score-class-selector', 'value'),
#      Input('grade-type-selector', 'value'), Input('score-exam-type-selector', 'value')],
#     [State('input-student-id', 'value')]
# )
...
#     [Input('controller-aspect-selector', 'value'), Input('controller-graph-table-selector', 'value'),
#      Input('controller-sta-term-selector', 'value')],
#     [State('input-student-id', 'value')],
# )
...
#             simple_table(table_data, '学生{0}{1}学期出勤情况统计'.format(stu_id, term))]
#
#
# @app.callback(
#     Output('student-consumption-show', 'children'),
#     [Input('consumption-graph-table-selector', 'value'), Input('interval-selector', 'value'),
#      Input('input-student-id', 'value')],
# )
